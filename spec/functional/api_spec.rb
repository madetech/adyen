...
if File.exist?(API_SPEC_INITIALIZER)
  ...
  describe Adyen::SOAP, "with an actual remote connection" do
    ...
    def perform_payment_request
      @payment_response = Adyen::SOAP.authorise_payment(
        @order_id,
    ...
    #   response = Adyen::SOAP.authorise_recurring_payment(
    #     @order_id,
    #     { :currency => 'EUR', :value => '1234' },
    #     { :email => "#{@user_id}@example.com", :reference => @user_id }
    #   )
    ...
    #   detail   = Adyen::SOAP.list_recurring_details(@user_id).references.last
    #   response = Adyen::SOAP.authorise_one_click_payment(
    #     @order_id,
    #     { :currency => 'EUR', :value => '1234' },
    #     { :email => "#{@user_id}@example.com", :reference => @user_id },
    #     '737',
    ...
    #it "stores the provided ELV account details" do
      #response = Adyen::SOAP.store_recurring_token(
        #{ :email => "#{@user_id}@example.com", :reference => @user_id },
    ...
    it "captures a payment" do
      response = Adyen::SOAP.capture_payment(@payment_response.psp_reference, { :currency => 'EUR', :value => '1234' })
      response.should be_success
    end

    it "refunds a payment" do
      response = Adyen::SOAP.refund_payment(@payment_response.psp_reference, { :currency => 'EUR', :value => '1234' })
      response.should be_success
    end

    it "cancels or refunds a payment" do
      response = Adyen::SOAP.cancel_or_refund_payment(@payment_response.psp_reference)
      response.should be_success
    end

    it "cancels a payment" do
      response = Adyen::SOAP.cancel_payment(@payment_response.psp_reference)
      response.should be_success
    end

    # TODO disabled for now: https://github.com/wvanbergen/adyen/issues/29
    # it "disables a recurring contract" do
    #   response = Adyen::SOAP.disable_recurring_contract(@user_id)
    #   response.should be_success
